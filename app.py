...
# Configure Page
st.set_page_config(
    page_title="QuLab: Data Quality, Provenance & Bias Assessment for Enterprise AI Systems", layout="wide")
st.sidebar.image("https://www.quantuniversity.com/assets/img/logo5.jpg")
st.sidebar.divider()
st.title("QuLab: Data Quality, Provenance & Bias Assessment for Enterprise AI Systems")
st.divider()

# --- Session State Initialization ---

if 'page' not in st.session_state:
    st.session_state.page = "Introduction & Data Load"  # Default page
...
if 'dataset_metadata' not in st.session_state:
    # Stores DatasetMetadata Pydantic object
...
if 'dq_report_hash' not in st.session_state:
    # SHA256 hash of the generated DQ report
    st.session_state.dq_report_hash = None

if 'bias_report_hash' not in st.session_state:
    # SHA256 hash of the generated Bias report
    st.session_state.bias_report_hash = None

if 'executive_summary_hash' not in st.session_state:
    # SHA256 hash of the generated Executive Summary
    st.session_state.executive_summary_hash = None

if 'evidence_manifest_hash' not in st.session_state:
    # SHA256 hash of the generated Evidence Manifest
    st.session_state.evidence_manifest_hash = None

if 'session_id' not in st.session_state:
    # Generate a unique session ID for this user
    st.session_state.session_id = hashlib.sha256(
        str(datetime.now(timezone.utc).timestamp()).encode()).hexdigest()[:16]

if 'output_dir' not in st.session_state:
    # Directory for saving reports with unique session hash
    st.session_state.output_dir = f"audit_artifacts_{st.session_state.session_id}"
...
if 'reference_groups' not in st.session_state:
    # Dict to store reference group per sensitive attribute
...
st.session_state.page = st.sidebar.selectbox(
    "Go to",
    ["Introduction & Data Load", "Data Quality Audit",
        "Bias Assessment", "Reports & Recommendations"]
)


# --- Page: Introduction & Data Load ---
if st.session_state.page == "Introduction & Data Load":
    ...
    if st.session_state.df is not None:
        ...
        st.session_state.label_column = 'is_fraud'
        ...
        st.session_state.sensitive_attributes = st.multiselect(
            ...
            options=[col for col in all_columns if col !=
        ...
        if st.button("Process Data & Capture Metadata"):
            with st.spinner("Capturing metadata and inferring schema..."):
                ...
                inferred_schema = {
                    col: str(dtype) for col, dtype in st.session_state.df.dtypes.items()}

                # Try to extract source_system and ingestion_date from first row, or use defaults
                source_system = st.session_state.df['source_system'].iloc[
                    0] if 'source_system' in st.session_state.df.columns else "Unknown"
                ingestion_date_str = str(st.session_state.df['ingestion_date'].iloc[0]) if 'ingestion_date' in st.session_state.df.columns else datetime.now(
                ...
                try:
                    ...
                    st.warning(
                        f"Could not parse 'ingestion_date' '{ingestion_date_str}'. Using current UTC datetime.")
                ...
                # Store the inferred schema for schema check
                st.session_state.expected_schema = inferred_schema
            ...
            with col1:
                st.metric("Total Records",
                          f"{st.session_state.dataset_metadata.record_count:,}")
            with col2:
                st.metric("Features", len(
                    st.session_state.dataset_metadata.schema))
            with col3:
                st.metric("Sensitive Attributes", len(
                    st.session_state.dataset_metadata.sensitive_attributes))
            with col4:
                st.metric("Source System",
                          st.session_state.dataset_metadata.source_system)
            ...
            with col1:
                st.info(
                    f"**Label Column:** `{st.session_state.dataset_metadata.label_column}`")
                st.info(
                    f"**Ingestion Date:** `{st.session_state.dataset_metadata.ingestion_date.strftime('%Y-%m-%d')}`")
            with col2:
                st.info(
                    f"**Sensitive Attributes:** {', '.join([f'`{attr}`' for attr in st.session_state.dataset_metadata.sensitive_attributes])}")

            # Schema details in an expander
            with st.expander("📊 View Dataset Schema"):
                schema_df = pd.DataFrame([
                    {"Column": col, "Data Type": dtype}
                    for col, dtype in st.session_state.dataset_metadata.schema.items()
                ])
        ...
        st.info("Please load demo data to proceed.")
...
# --- Page: Data Quality Audit ---
    ...
    st.markdown(f"")
    st.markdown(
        r"The **missing value rate** for a column $C$ in a dataset with $N$ records is given by:")
    st.markdown(
        r"$$ \text{{Missing Rate}}(C) = \frac{{\text{{Number of Missing Values in }} C}}{{N}} $$")
    st.markdown(
        r"where $C$ is a column and $N$ is the total number of records in the dataset.")
    st.markdown(f"")
    st.markdown(
        r"The **duplicate row count** for a dataset is simply the number of rows that are exact copies of other rows.")
    st.markdown(f"")

    if st.session_state.df is None:
        st.warning(
            "Please load data in the 'Introduction & Data Load' section first.")
    else:
        ...
        st.markdown(f"")
        st.markdown(
            f"## 3. Data Quality Audit: Outliers and Schema Consistency")
        st.markdown(f"")
        ...
        if st.button("Run All Data Quality Checks"):
            ...
            with st.spinner("Performing missing value checks..."):
                st.session_state.dq_findings.extend(
                    perform_missing_value_check(
                        st.session_state.df, threshold=st.session_state.dq_threshold_missingness)
                )
            with st.spinner("Performing duplicate checks..."):
                st.session_state.dq_findings.append(
                    perform_duplicate_check(
                        st.session_state.df, threshold=st.session_state.dq_threshold_duplicates)
                )
            with st.spinner("Performing outlier checks..."):
                ...
                for col in numerical_cols:
                    ...
                    if col not in ['transaction_id', st.session_state.label_column]:
                        st.session_state.dq_findings.append(
                            perform_outlier_check_iqr(
                                st.session_state.df, col, threshold_iqr_multiplier=iqr_multiplier, outlier_threshold=st.session_state.dq_threshold_outliers)
                        )
            with st.spinner("Performing schema consistency checks..."):
                if st.session_state.expected_schema:
                    st.session_state.dq_findings.extend(
                        perform_schema_check(
                            st.session_state.df, st.session_state.expected_schema)
                    )
                else:
                    st.warning(
                        "Expected schema not defined. Please process data in 'Introduction & Data Load' first.")

            st.success("Data Quality Checks Complete!")

        if st.session_state.dq_findings:
            st.subheader("Data Quality Findings Summary")
            dq_df = pd.DataFrame([f.model_dump()
                                 for f in st.session_state.dq_findings])
            dq_df_display = dq_df[[
                'issue_type', 'column', 'metric_value', 'threshold', 'status', 'description']]
            st.dataframe(dq_df_display)

            overall_dq_status = "PASS" if all(
                f.status == "PASS" for f in st.session_state.dq_findings if f.status != "N/A") else "FAIL"
            st.metric(label="Overall Data Quality Status",
                      value=overall_dq_status)
            ...
            missing_findings = [
                f for f in st.session_state.dq_findings if f.issue_type == "MISSINGNESS"]
            duplicate_findings = [
                f for f in st.session_state.dq_findings if f.issue_type == "DUPLICATES"]
            outlier_findings = [
                f for f in st.session_state.dq_findings if f.issue_type == "OUTLIERS"]
            schema_findings = [
                f for f in st.session_state.dq_findings if f.issue_type == "SCHEMA_DRIFT"]

            # Missing values explanation
            if missing_findings:
                fail_missing = [
                    f for f in missing_findings if f.status == "FAIL"]
                if fail_missing:
                    cols = ', '.join(
                        [f"`{f.column}`" for f in fail_missing[:3]])
                    st.markdown(f"- The audit reveals that {cols} {'column has' if len(fail_missing) == 1 else 'columns have'} significant missing value rates exceeding GlobalTrust's threshold, resulting in **FAIL** status. Sarah knows this means imputations or advanced missing value handling strategies are required before training, as simply dropping these rows could remove valuable data or introduce further bias.")
                else:
                    st.markdown(f"- The missing value check shows **PASS** status for all columns, indicating that missing data rates are within acceptable limits. This demonstrates good data collection practices and reduces the need for complex imputation strategies.")
                st.markdown(f"")

            # Duplicates explanation
            if duplicate_findings:
                dup_status = duplicate_findings[0].status
                dup_rate = duplicate_findings[0].metric_value
                if dup_status == "FAIL":
                    st.markdown(
                        f"- The check identified duplicate rows ({dup_rate:.2%}), exceeding the duplicate threshold. This finding is a **FAIL** and signals that the dataset needs de-duplication. Ignoring duplicates would lead to the model over-learning from redundant data points, potentially skewing feature importance and performance metrics.")
                else:
                    st.markdown(
                        f"- The duplicate check shows **PASS** status with only {dup_rate:.2%} duplicate rows, which is within acceptable limits. This indicates good data quality and reduces the risk of biased model training from redundant records.")
                st.markdown(f"")

            # Outliers explanation
            if outlier_findings:
                fail_outliers = [
                    f for f in outlier_findings if f.status == "FAIL"]
                if fail_outliers:
                    cols = ', '.join(
                        [f"`{f.column}`" for f in fail_outliers[:3]])
                    st.markdown(f"- The outlier check for {cols} {'column reports' if len(fail_outliers) == 1 else 'columns report'} **FAIL** status. Sarah notes that these outliers, representing extreme values, could either be legitimate but rare events, or indicators of data errors or unusual activity. This requires further investigation. Depending on their nature, Winsorization or robust scaling might be necessary.")
                else:
                    st.markdown(f"- The outlier checks show **PASS** status for all numerical columns, indicating that extreme values are within acceptable thresholds. This suggests the data distribution is relatively stable and won't require aggressive outlier treatment.")
                st.markdown(f"")

            # Schema consistency explanation
            if schema_findings:
                fail_schema = [
                    f for f in schema_findings if f.status == "FAIL"]
                if fail_schema:
                    cols = ', '.join(
                        [f"`{f.column}`" for f in fail_schema[:3]])
                    st.markdown(
                        f"- The schema consistency check flags **FAIL** status for {cols}, indicating data type mismatches. Sarah understands that schema drift is a critical issue that can silently break production models. These deviations must be corrected to ensure the data will fit the model's input requirements.")
                else:
                    st.markdown(f"- The schema consistency check reports **PASS** status, confirming that all data types match expectations. This reassures Sarah that the data will fit the model's input requirements without type conversion errors in the production pipeline.")
            else:
                st.markdown(f"- The schema consistency check reports **PASS** status, confirming that all data types match expectations. This reassures Sarah that the data will fit the model's input requirements without type conversion errors in the production pipeline.")
...
# --- Page: Bias Assessment ---
    ...
    st.markdown(f"")
    st.markdown(
        f"## 4. Bias Assessment: Demographic Parity and Disparate Impact")
    st.markdown(f"")
    ...
    st.markdown(
        r"$$ DPD = P(\text{{Positive Outcome}} | \text{{Protected Group}}) - P(\text{{Positive Outcome}} | \text{{Reference Group}}) $$")
    st.markdown(
        r"where $P(\text{{Positive Outcome}} | \text{{Group}})$ is the probability of a positive outcome for a given group.")
    st.markdown(
        r"A common threshold for DPD is that its absolute value should be less than 0.1, i.e., $|DPD| \le 0.1$.")
    st.markdown(f"")
    ...
    st.markdown(
        r"$$ DIR = \frac{{P(\text{{Positive Outcome}} | \text{{Protected Group}})}}{{P(\text{{Positive Outcome}} | \text{{Reference Group}})}} $$")
    st.markdown(
        r"where $P(\text{{Positive Outcome}} | \text{{Group}})$ is the probability of a positive outcome for a given group.")
    st.markdown(
        r"A common threshold for DIR is the '80% rule,' where $0.8 \le DIR \le 1.25$.")
    st.markdown(f"")

    if st.session_state.df is None or not st.session_state.sensitive_attributes or st.session_state.label_column is None:
        st.warning(
            "Please load data and define Label Column & Sensitive Attributes in the 'Introduction & Data Load' section first.")
    else:
        ...
        POSITIVE_LABEL_VALUE = st.radio(
            ...
            options=st.session_state.df[st.session_state.label_column].unique(
            ).tolist(),
        ...
        for attr in st.session_state.sensitive_attributes:
            unique_groups = st.session_state.df[attr].dropna().astype(
                str).unique().tolist()
            if unique_groups:
                ...
                st.session_state.reference_groups[attr] = st.selectbox(
                    ...
                    index=unique_groups.index(
                        default_ref) if default_ref in unique_groups else 0,
                    key=f"ref_group_{attr}"
                )
            else:
                st.warning(
                    f"No unique groups found for sensitive attribute '{attr}'. Skipping reference group selection.")
        ...
        if st.button("Run Bias Assessment"):
            ...
            for attr in st.session_state.sensitive_attributes:
                if attr in st.session_state.reference_groups:
                    ...
                    st.markdown(
                        f"Running bias checks for **'{attr}'** with reference group **'{ref_group}'**...")
                    ...
                    st.warning(
                        f"No reference group selected for sensitive attribute '{attr}'. Skipping bias calculation.")

            st.success("Bias Assessment Complete!")

        if st.session_state.bias_results:
            st.subheader("Bias Assessment Results")
            bias_df = pd.DataFrame([b.model_dump()
                                   for b in st.session_state.bias_results])
            bias_df_display = bias_df[['metric_name', 'sensitive_attribute',
                                       'group', 'reference_group', 'value', 'status', 'interpretation']]
            st.dataframe(bias_df_display)

            # Add visualizations
            st.markdown("### 📊 Bias Metrics Visualizations")

            # Separate DPD and DIR results
            dpd_results = bias_df[bias_df['metric_name']
                                  == 'DEMOGRAPHIC_PARITY']
            dir_results = bias_df[bias_df['metric_name'] == 'DISPARATE_IMPACT']

            col1, col2 = st.columns(2)

            with col1:
                if not dpd_results.empty:
                    st.markdown("#### Demographic Parity Difference (DPD)")
                    # Create bar chart for DPD
                    fig_dpd = go.Figure()

                    for attr in dpd_results['sensitive_attribute'].unique():
                        attr_data = dpd_results[dpd_results['sensitive_attribute'] == attr]
                        colors = [
                            'red' if status == 'FAIL' else 'green' for status in attr_data['status']]

                        fig_dpd.add_trace(go.Bar(
                            name=attr,
                            x=attr_data['group'],
                            y=attr_data['value'],
                            marker_color=colors,
                            text=attr_data['status'],
                            textposition='auto',
                            hovertemplate='<b>%{x}</b><br>DPD: %{y:.3f}<br>Status: %{text}<extra></extra>'
                        ))

                    # Add threshold lines
                    fig_dpd.add_hline(y=st.session_state.bias_dpd_threshold_max,
                                      line_dash="dash", line_color="orange",
                                      annotation_text=f"Upper Threshold ({st.session_state.bias_dpd_threshold_max})")
                    fig_dpd.add_hline(y=st.session_state.bias_dpd_threshold_min,
                                      line_dash="dash", line_color="orange",
                                      annotation_text=f"Lower Threshold ({st.session_state.bias_dpd_threshold_min})")
                    fig_dpd.add_hline(y=0, line_dash="solid", line_color="gray",
                                      annotation_text="Ideal (0)")

                    fig_dpd.update_layout(
                        title="Demographic Parity Difference by Group",
                        xaxis_title="Group",
                        yaxis_title="DPD Value",
                        barmode='group',
                        height=400,
                        showlegend=True
                    )
            ...
            with col2:
                if not dir_results.empty:
                    st.markdown("#### Disparate Impact Ratio (DIR)")
                    # Create bar chart for DIR
                    fig_dir = go.Figure()

                    for attr in dir_results['sensitive_attribute'].unique():
                        attr_data = dir_results[dir_results['sensitive_attribute'] == attr]
                        colors = [
                            'red' if status == 'FAIL' else 'green' for status in attr_data['status']]
                        ...
                        fig_dir.add_trace(go.Bar(
                            ...
                            text=attr_data['status'],
                            textposition='auto',
                            hovertemplate='<b>%{x}</b><br>DIR: %{y:.3f}<br>Status: %{text}<extra></extra>'
                        ))

                    # Add threshold lines
                    fig_dir.add_hline(y=st.session_state.bias_dir_threshold_max,
                                      line_dash="dash", line_color="orange",
                                      annotation_text=f"Upper Threshold ({st.session_state.bias_dir_threshold_max})")
                    fig_dir.add_hline(y=st.session_state.bias_dir_threshold_min,
                                      line_dash="dash", line_color="orange",
                                      annotation_text=f"Lower Threshold ({st.session_state.bias_dir_threshold_min})")
                    fig_dir.add_hline(y=1.0, line_dash="solid", line_color="gray",
                                      annotation_text="Ideal (1.0)")

                    fig_dir.update_layout(
                        title="Disparate Impact Ratio by Group",
                        xaxis_title="Group",
                        yaxis_title="DIR Value",
                        barmode='group',
                        height=400,
                        showlegend=True
                    )
            ...
            # Status summary heatmap
            st.markdown("#### Bias Assessment Status Summary")

            # Create a pivot table for status
            status_pivot = bias_df.pivot_table(
            ...
            fig_heatmap = go.Figure(data=go.Heatmap(
                ...
                texttemplate='%{text} FAILs',
                textfont={"size": 12},
                hovertemplate='Attribute: %{y}<br>Metric: %{x}<br>Failures: %{z}<extra></extra>'
            ))

            fig_heatmap.update_layout(
                title="Number of FAIL statuses by Sensitive Attribute and Metric",
                xaxis_title="Bias Metric",
                yaxis_title="Sensitive Attribute",
                height=300
            )
            ...
            # Generate dynamic explanation based on actual findings
            fail_results = bias_df[bias_df['status'] == 'FAIL']
            pass_results = bias_df[bias_df['status'] == 'PASS']

            if not fail_results.empty:
                # Group failures by sensitive attribute
                for attr in fail_results['sensitive_attribute'].unique():
                    attr_fails = fail_results[fail_results['sensitive_attribute'] == attr]
                    fail_groups = attr_fails['group'].unique()
                    ref_group = attr_fails['reference_group'].iloc[0]

                    fail_metrics = attr_fails['metric_name'].unique()
                    metric_names = []
                    if 'DEMOGRAPHIC_PARITY' in fail_metrics:
                        metric_names.append("Demographic Parity Difference")
                    if 'DISPARATE_IMPACT' in fail_metrics:
                        metric_names.append("Disparate Impact Ratio")

                    groups_str = ', '.join([f"'{g}'" for g in fail_groups[:3]])
                    if len(fail_groups) > 3:
                        groups_str += f", and {len(fail_groups) - 3} more"

                    st.markdown(f"- **{attr}**: The {' and '.join(metric_names)} for {groups_str} compared to the '{ref_group}' reference group report **FAIL** statuses. This indicates disproportionate fraud flagging rates in the training data. This finding is a high-risk concern for GlobalTrust, potentially leading to unfair treatment and violating anti-discrimination regulations.")
                    st.markdown(f"")

                st.markdown(f"Sarah recognizes that these biases in the training data, if not mitigated, will be learned and amplified by the *TransactionGuard AI* model, leading to discriminatory outcomes. This necessitates further investigation into the source of these disparities and potential data re-sampling or re-weighting strategies to promote fairness.")
            else:
                st.markdown(f"- **Excellent news**: All bias metrics report **PASS** status across all sensitive attributes! The Demographic Parity Differences and Disparate Impact Ratios are within acceptable thresholds, indicating that the training data does not exhibit systematic bias against any demographic groups.")
                st.markdown(f"")
                st.markdown(f"This demonstrates that GlobalTrust's data collection and processing practices have successfully maintained fairness. Sarah can confidently report to the AI Governance Committee that the *TransactionGuard AI* model is trained on balanced data, significantly reducing the risk of discriminatory outcomes.")
...
# --- Page: Reports & Recommendations ---
    ...
    st.markdown(
        r"$$ \text{{SHA256 Hash}} = \text{{hashlib.sha256}}(\text{{data.encode()}}).\text{{hexdigest}}() $$")
    st.markdown(
        r"where `data` is the string representation of the content to be hashed.")
    st.markdown(f"")
    ...
    if st.session_state.df is None or not st.session_state.dq_findings or not st.session_state.bias_results or st.session_state.dataset_metadata is None:
        st.warning(
            "Please complete 'Introduction & Data Load', 'Data Quality Audit', and 'Bias Assessment' sections first.")
    else:
        ...
        if st.button("Generate All Reports and Evidence Manifest"):
            with st.spinner("Generating Data Quality Report..."):
                st.session_state.dq_report_hash = generate_data_quality_report_json(
                    ...
                    os.path.join(st.session_state.output_dir,
            ...
            with st.spinner("Generating Executive Summary..."):
                st.session_state.executive_summary_hash = generate_executive_summary_md(
                    ...
                    output_path=os.path.join(
                        st.session_state.output_dir, EXECUTIVE_SUMMARY_FILENAME)
                )
            ...
            with st.spinner("Generating Evidence Manifest..."):
                ...
                st.session_state.evidence_manifest_hash = generate_evidence_manifest_json(
                    ...
                    output_path=os.path.join(
                        st.session_state.output_dir, EVIDENCE_MANIFEST_FILENAME)
                )
                ...
                st.session_state.evidence_manifest_hash = calculate_sha256_hash(
        ...
        if st.session_state.evidence_manifest_hash:
            ...
            st.markdown(
                f"**Data Quality Report Hash:** `{st.session_state.dq_report_hash}`")
            st.markdown(
                f"**Bias Metrics Report Hash:** `{st.session_state.bias_report_hash}`")
            st.markdown(
                f"**Executive Summary Hash:** `{st.session_state.executive_summary_hash}`")
            st.markdown(
                f"**Evidence Manifest Hash:** `{st.session_state.evidence_manifest_hash}`")

            st.markdown(f"")
            st.markdown(
                f"#### Explanation of Execution: Reports and Evidence Manifest")
            ...
            st.markdown(
                f"1. **Provenance**: Any future review can verify exactly which dataset was audited.")
            st.markdown(
                f"2. **Integrity**: It can be proven that the reports have not been tampered with since their generation.")
            st.markdown(f"")
            ...
            st.markdown(f"")
            st.markdown(
                f"## 6. Executive Summary and Mitigation Recommendations")
            st.markdown(f"")
            ...
            if os.path.exists(executive_summary_path):
                ...
                # Display in a bounded container
            ...
            with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zf:
                for root, _, files in os.walk(st.session_state.output_dir):
                    for file in files:
                        ...
                        zf.write(file_path, arcname=os.path.relpath(
                            file_path, st.session_state.output_dir))

            # Reset buffer position to the beginning
            zip_buffer.seek(0)

            def cleanup_and_reset():
                """Delete the artifacts directory and reset evidence-related state"""
